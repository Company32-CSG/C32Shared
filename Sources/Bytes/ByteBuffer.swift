...
public extension C32 {
	...
	class ByteBuffer {
		...
		public func split(from: Int, to: Int) -> ByteBuffer? {
			guard from < _data.count, from + to < _data.count else { return nil }
			
			return ByteBuffer(Array(_data[from...(from+to)]))
		}
		...
		public func read(_ length: UInt8, _ moveCursor: Bool = true) -> ByteBuffer? {
			return readBytes(Int(length), moveCursor)
		}
		
		public func read(_ length: UInt16, _ moveCursor: Bool = true) -> ByteBuffer? {
			return readBytes(Int(length), moveCursor)
		}
		
		public func read(_ length: UInt32, _ moveCursor: Bool = true) -> ByteBuffer? {
			return readBytes(Int(length), moveCursor)
		}
		
		public func read(_ length: Int, _ moveCursor: Bool = true) -> ByteBuffer? {
			return readBytes(Int(length), moveCursor)
		}
		...
		public static func +=(lhs: ByteBuffer, rhs: UInt8) {
			lhs._data.append(rhs)
		}
		
		public static func +=(lhs: ByteBuffer, rhs: UInt16) {
			lhs._data.append(contentsOf: [UInt8((rhs >> 8) & 0xFF), UInt8(rhs & 0xFF)])
		}
		
		public static func +=(lhs: ByteBuffer, rhs: UInt32) {
		...
		public static func +=(lhs: ByteBuffer, rhs: Data) {
			lhs._data.append(contentsOf: rhs)
		}
		
		public static func +=(lhs: ByteBuffer, rhs: ByteBuffer) {
		...
		private func readBytes(_ length: Int, _ moveCursor: Bool = true) -> ByteBuffer? {
			guard length > 0 else {
				return ByteBuffer()
			}
			...
			let retval = ByteBuffer(Array(_data[_pos...(_pos + length - 1)]))
...
extension C32.ByteBuffer: Sequence {
